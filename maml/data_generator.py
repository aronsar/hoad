...
class OmniglotDataGenerator(object):
    ...
    def sample_batch(self, is_train=True, is_eval=False):
        ...
        def _multiprocess_batching(task_ids_list):
            ...
            eval_batch = []
            if is_train:
                config = [(self.num_shots+1, self.num_classes)] * \
                    self.num_tasks
                train_args = zip(task_ids_list, config)

                with mp.Pool(self.num_process) as pool:
                    train_batch = pool.starmap(sample_task_batch,
                                               train_args)

            if is_eval:
                config = [(1, self.num_classes)] * self.num_tasks
                with mp.Pool(self.num_process) as pool:
                    eval_batch = pool.starmap(sample_task_batch,
                                              zip(task_ids_list, config))
            ...
            for task in range(self.num_tasks):
                ...
                if is_train:
                    x_train_task_batch, y_train_task_batch = sample_task_batch(
                        task_ids, (self.num_shots+1, self.num_classes))
                    train_batch.append(
                        (x_train_task_batch, y_train_task_batch))

                if is_eval:
                    x_eval_task_batch, y_eval_task_batch = sample_task_batch(
                        task_ids, (1, self.num_classes))

                    eval_batch.append((x_eval_task_batch, y_eval_task_batch))
