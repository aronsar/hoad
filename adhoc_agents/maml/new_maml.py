...
class MAML(object):
    ...
    @tf.function
    def train_support_step(self, task, model):
        # Retrace Makes the code 20 times slower
        print(f"##### Retrace train_support {task} #####")

        # Step 1: Forward Pass
        with tf.GradientTape() as task_tape:
            predictions = model(self.x_support)
            loss = self.task_loss_op(self.y_support, predictions)
    ...
    @tf.function
    def train_query_step(self, task, model):
        # Query Set
        print(f"##### Retrace train_query {task} #####")

        # Step 1: Forward Pass
        with tf.GradientTape() as task_tape:
            predictions = model(self.x_query)
            loss = self.task_loss_op(self.y_query, predictions)
        ...
        self.task_loss(loss)
        self.task_accuracy(self.y_query, predictions)

        return grads

    @tf.function
    def eval_support_step(self, task, model):
        # Retrace Makes the code 20 times slower
        print(f"##### Retrace eval_support {task} #####")
    ...
    @tf.function
    def eval_query_step(self, task, model):
        # Query Set
        print(f"##### Retrace eval_query {task} #####")
        # Step 1: Forward Pass
        with tf.GradientTape() as task_tape:
            predictions = model(self.x_query)
            loss = self.task_loss_op(self.y_query, predictions)
        ...
        # Step 2: Record Gradients for Meta Gradients
        self.eval_loss(loss)
        self.eval_accuracy(self.y_query, predictions)
    ...
    def train_step(self, train_batch, train_classes, step):
        ...
        for task in range(self.num_tasks):
            ...
            for i in range(len(x_support)):
                self.x_support = x_support[i]
                self.y_support = y_support[i]
                self.train_support_step(task, self.task_models[task])

            grads = None
            for j in range(len(x_query)):
                self.x_query = x_query[j]
                self.y_query = y_query[j]
                grads = self.train_query_step(task, self.task_models[task])
    ...
    def eval_step(self, eval_batch, eval_classes, step):
        ...
        for task in range(len(eval_batch)):
            ...
            for i in range(len(x_support)):
                self.x_support = x_support[i]
                self.y_support = y_support[i]
                self.eval_support_step(task, self.task_models[task])

            for j in range(len(x_query)):
                self.x_query = x_query[j]
                self.y_query = y_query[j]
                self.eval_query_step(task, self.task_models[task])
